...
```bash
docker run --rm -v "$PWD":/w -w /w python:3.12-slim python3 duui-video-emotion-visualization-webapp/tests/contrast_check.py
```

The pytest gate that enforces it:

```bash
docker run --rm -v "$PWD":/w -w /w python:3.12-slim sh -c "pip install -q pytest psycopg2-binary && pytest duui-video-emotion-visualization-webapp/tests/test_contrast.py -q"
```
