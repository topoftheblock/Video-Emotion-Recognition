...
an import is running and how far along it is.

Three properties this module is built around:

1. **It never breaks the job.** Reporting progress is a convenience;
   an import that fails because the status table was missing or the
...
2. **It needs its own connection.** pipeline.py opens a connection per
   file and only around the insert -- during the CAS parse and the
   video copy, which is most of the wall time, the job holds no
   connection at all. This one is opened once for the whole run and is
   autocommit, so each heartbeat is visible to the viewer immediately
   rather than at the end of a transaction nobody is waiting on.
...
seconds. The phases that report progress are not the phases that take
the time -- loading a large CAS is one call that can run for minutes --
so without it the viewer would call a healthy import dead.
...
the stack (see this repo's Dockerfiles). db/schema.sql carries the
...
class JobRun:
    ...
    def _give_up(self, exc):
        self._disabled = True
        self._stop_beating.set()
        print(f"[duui_parser] job status reporting disabled ({exc})")
        if self._conn is not None:
            try:
                self._conn.close()
            except Exception:  # noqa: BLE001
