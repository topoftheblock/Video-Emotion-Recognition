"""Create the empty test database if it is missing, and apply the schema.

The suite needs a database with the schema and no rows: the DB-backed tests
assume they are the only writer, and several of them fail against a populated
corpus rather than skipping. Nothing else creates that database — losing it to a
rebuild is what silently turned a 150-pass run into 121 passed and 29 skipped
once already.
