...
**Almost nothing here is a test.** The four sub-projects hold those, each in
...
once — and, since Phase 8, one test file covering the checkers themselves.
...
`stylecheck.py` is the only enforcement several style-guide rules have, so a
bug in it is silent — the check passes and the rule quietly stops existing.
That is why it has tests, and why they run with everything else.
