...
-- Read before adding a table: everything imported from a CAS is keyed
-- by (video_id, <the CAS's own xmi:id>), never by the xmi:id alone. An
-- xmi:id is unique within one document and means nothing across
-- documents, so the same id names different things in different videos.
--
-- `models` and `global_persons` are the two deliberate exceptions: both
-- are corpus-wide and neither takes its id from a CAS.
--
...
CREATE TABLE videos (
    ...
    -- The importer upserts on this, so a video re-imported any number
    -- of times keeps one row. It is also the join key to the video
    -- store: the webapp plays `<DUUI_VIDEO_DIR>/<filename>`.
...
CREATE TABLE persons (
    ...
    global_person_id BIGINT
        REFERENCES global_persons(global_person_id) ON DELETE SET NULL,
    clip_label TEXT,
...
CREATE TABLE segments (
    ...
    -- state (a shot has no speaker), which is also why this FK is MATCH
    -- SIMPLE — a NULL person_id leaves it unenforced.
...
-- for `<=>`, which is what identity linking compares centroids with
-- (global-identity-linker/src/identity/linking.py) and what any
-- similarity search the Ask panel generates will use.
...
CREATE TABLE base_emotions (
    ...
    -- Often NULL, in every modality: a reading the upstream pipeline
    -- could not attribute to a person still belongs to the video.
    person_id BIGINT,
...
-- Not required for correctness. These back the filters identity linking
-- runs over the whole corpus
-- (global-identity-linker/src/identity/linking.py) and the ones the Ask
-- panel's generated SQL tends to use.
...
-- The referencing side of each composite foreign key. Postgres indexes
-- the referenced side automatically, not this one, so without these a
-- cascade deleting by person or presence scans. `--on-existing replace`
-- deletes a whole video's subtree on every re-import, which is when
-- that matters.
...
-- 7. Job status
-- One row per job run, so the webapp can report that an import is
-- running and how far it has got, rather than leaving an empty video
-- list ambiguous.
--
...
-- repeated as CREATE TABLE IF NOT EXISTS in the three services that
-- write or read the table. The four are identical apart from the
-- statement terminator, which only this file needs; keep them so:
...
--
-- fillfactor leaves free space on each page for the heartbeat UPDATEs,
-- which rewrite one row roughly once a second for the length of a run
-- (MIN_WRITE_INTERVAL in job_runs.py). With room on the page those
-- updates stay HOT and leave no dead tuple or new index entry behind.
