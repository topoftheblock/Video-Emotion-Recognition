 ...
 * register with the player's onFrame() and are handed (payload,
 * currentTime) on every animation frame while playing, and once per
 * seek/timeupdate while paused. They then pick out whichever rows of
 * the payload cover that instant — subtitle text, the text and audio
 * emotion readouts, and bounding boxes for every detection near this
 * time.
 *
 * This module does the wiring and nothing else, which is what keeps the
 * renderers from having to import each other.
 */
