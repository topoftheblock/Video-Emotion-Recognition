...
async function a11yCheck({
  axeUrl = "https://cdn.jsdelivr.net/npm/axe-core@4.10.2/axe.min.js",
} = {}) {
  const TAGS = [
    "wcag2a",
    "wcag2aa",
  ...
  stub({
    ...
    rows: [
      { filename: "a.mp4", person_count: 3 },
      { filename: "b.mp4", person_count: 5 },
    ],
    row_count: 2,
    truncated: true,
    sql: "SELECT ...",
  ...
  results.E.combobox = {
    ...
    ariaSelected: document.querySelectorAll('#videoComboList li[aria-selected="true"]')
      .length,
  };
  ...
  results.summary = {
    totalViolations: ["A", "B", "C", "D", "E"].reduce(
      (n, k) => n + results[k].violations.length,
      0,
    ),
...
function a11ySweep() {
  ...
  const rows = nodes.map((n, i) => {
    ...
    const attr = (a) =>
      n.getAttribute(a) !== null
        ? `${a.replace("aria-", "")}=${n.getAttribute(a)}`
        : null;
    return {
      stop: i + 1,
      region: n.closest("header")
        ? "header"
        : n.closest(".ask-panel")
          ? "ask"
          : n.closest(".stage")
      ...
      name,
      state:
        ["aria-pressed", "aria-expanded", "aria-valuetext", "aria-disabled"]
          .map(attr)
          .filter(Boolean)
          .join(" ") || "-",
    };
  ...
  return {
    ...
    positiveTabindex: document.querySelectorAll(
