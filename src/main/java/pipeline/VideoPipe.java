...
public class VideoPipe {
    ...
    public static void main(String[] args) throws Exception {
        ...
        composer.addDriver(new DUUIDockerDriver(),new DUUIRemoteDriver(), new DUUIUIMADriver().withDebug(true));
        ...
        // 5) pipeline.EmotionSentenceMapper: DUUI-Emotion -> eigenes Schema, mit AudioSentence verknüpfen
        composer.add(new DUUIUIMADriver.Component(
                createEngineDescription(EmotionSentenceMapper.class,
                        EmotionSentenceMapper.PARAM_VIEW, TRANSCRIPT_VIEW)));

        // 6) Video-Segmente mit eingebranntem Untertitel schreiben (braucht _InitialView + AudioSentence)
        composer.add(new DUUIUIMADriver.Component(
        ...
        DUUIMultimodalCollectionReader reader =
                new DUUIMultimodalCollectionReader(videoDir, "mp4", VIDEO_VIEW);
        ...
        if (reader.hasNext()) {
            ...
            try (java.io.FileOutputStream fos =
                         new java.io.FileOutputStream(new File(outDir, "transcript.xmi"))) {
                XmiCasSerializer.serialize(clean.getCas(), fos);
            }

            // passendes Typesystem daneben schreiben
            try (java.io.FileOutputStream ts =
                         new java.io.FileOutputStream(new File(outDir, "typesystem.xml"))) {
    ...
    private static JCas createCleanCas(JCas fullCas) throws Exception {
        ...
        // nur transcript-View (inkl. Sofa-Text) kopieren – Video-View (_InitialView) bleibt weg
